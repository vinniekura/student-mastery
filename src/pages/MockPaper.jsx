    ...
    <div style={{ background: 'var(--bg2)', border: '2px solid var(--teal-border)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div>
          ...
          <input style={inp} value={term} onChange={e => setTerm(e.target.value)} placeholder="e.g. Term 1, Semester 2, UCAT prep" />
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 11, color: 'var(--text3)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Topics covered — edit or add, comma separated
        </label>
        <textarea
          style={{ ...inp, height: 72, resize: 'vertical', lineHeight: 1.5 }}
          value={topics}
          onChange={e => setTopics(e.target.value)}
          placeholder="e.g. Electric fields, Magnetic fields, Gravitational fields, Capacitors"
        />
        {/* Topic pills for quick view */}
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <button
          ...
          style={{ padding: '10px 24px', borderRadius: 10, fontSize: 14, fontWeight: 600, background: 'var(--teal)', color: '#fff', border: 'none', cursor: 'pointer' }}
        >
        ...
        <button
          onClick={onReanalyse}
          disabled={analysing}
          style={{ padding: '10px 16px', borderRadius: 10, fontSize: 13, background: 'var(--bg3)', color: 'var(--text2)', border: '1px solid var(--border)', cursor: 'pointer' }}
        >
...
export default function MockPaper() {
  ...
  const [scope, setScope]                 = useState(null)         // saved scope from Redis
  ...
  async function loadScope() {
    try {
      const token = await getToken()
      const res = await fetch(`/api/analyse-docs?subjectId=${selectedSubjectId}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  async function runAnalysis() {
    ...
    try {
      const token = await getToken()
      const res = await fetch(`/api/analyse-docs?subjectId=${selectedSubjectId}`, {
        method: 'POST',
  ...
  async function clearScope() {
    try {
      const token = await getToken()
      await fetch(`/api/analyse-docs?subjectId=${selectedSubjectId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
      setScope(null)
    } catch {}
  }
    ...
    try {
      ...
      const res = await fetch('/api/generate-mock', {
        ...
        body: JSON.stringify({
          ...
          forceNew,
          replaceSlot,
          confirmedScope: scope?.confirmed ? scope : null  // pass confirmed scope to worker
        })
      })
      const data = await res.json()
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      {selectedSubjectId && (
        <>
          ...
          {!hasDocs && (
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, marginBottom: 20, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 16 }}>
                Upload your past papers and notes in the Subjects page first.<br />
                The more you upload, the better scoped your mock will be.
              </div>
              <div style={{ fontSize: 12, color: 'var(--text3)' }}>
                Go to Subjects → select {selectedSubject?.name} → upload past papers and notes
              </div>
            </div>
          )}

          {/* ── STEP 2: Docs exist, no scope yet → show Analyse button ── */}
          {hasDocs && !scope && (
            <div style={{ background: 'var(--bg2)', border: '2px solid var(--teal-border)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 16 }}>
                ...
                {' '}{subjectDocs.filter(d => d.docType === 'notes').length} notes).
                Analyse them all together to detect the term, topics and exam format — then confirm before generating.
              </div>
              ...
              <button
                onClick={runAnalysis}
                disabled={analysing}
                style={{ padding: '11px 28px', borderRadius: 10, fontSize: 14, fontWeight: 600, background: analysing ? 'var(--bg3)' : 'var(--teal)', color: analysing ? 'var(--text2)' : '#fff', border: 'none', cursor: analysing ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
              >
                {analysing
                  ? <><svg style={{ width: 14, height: 14, border: '2px solid var(--border)', borderTopColor: 'var(--text2)', borderRadius: '50%', animation: 'spin .8s linear infinite', flexShrink: 0 }} />Analysing your documents...</>
          ...
          {/* ── STEP 3: Scope detected, needs confirmation ── */}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, padding: '10px 14px', background: 'var(--teal-bg)', border: '1px solid var(--teal-border)', borderRadius: 10 }}>
              <div style={{ flex: 1 }}>
                ...
                {scope.topics?.length > 0 && (
                  <span style={{ fontSize: 11, color: 'var(--text3)', marginLeft: 8 }}>{scope.topics.slice(0, 4).join(' · ')}{scope.topics.length > 4 ? ` +${scope.topics.length - 4}` : ''}</span>
                )}
              </div>
          ...
          {/* ── STEP 4: Generate (only when scope confirmed) ── */}
          {scopeConfirmed && (
            <>
